    ...
    setupEventListeners() {
        ...
        if (mobileMenuBtn && navMenu) {
            mobileMenuBtn.addEventListener('click', () => {
                // Toggle using inline style instead of classes
    ...
    async handleLeadsSubmit(e) {
        ...
        try {
            ...
            if (response.success) {
                ...
                resultsContent.innerHTML = `
                    <div style="background: #FFFFFF; padding: 1.5rem; border-radius: var(--radius-lg); text-align: center; border: 1px solid var(--border); box-shadow: var(--shadow-md);">
                        <i class="fas fa-check-circle" style="color: var(--success); font-size: 3rem; margin-bottom: 1rem;"></i>
                        <h4 style="color: var(--gray-900); margin-bottom: 0.5rem;">Workflow Active</h4>
                        <p style="color: var(--gray-600); margin-bottom: 1.5rem;">
                            Your automated lead generation for <strong>"${searchQuery} in ${location}"</strong> has started.
                        </p>
                        
                        <div style="text-align: left; background: var(--gray-50); padding: 1rem; border-radius: var(--radius-md); border: 1px solid var(--border); margin-bottom: 1rem;">
                            <p style="color: var(--gray-800);"><strong>Workflow ID:</strong> ${response.data.workflowId}</p>
                            <p style="color: var(--gray-800);"><strong>Status:</strong> <span style="color: var(--primary);">${response.data.status}</span></p>
                            <p style="color: var(--gray-800);"><strong>Estimated Time:</strong> ${response.data.estimatedTime}</p>
                        </div>
